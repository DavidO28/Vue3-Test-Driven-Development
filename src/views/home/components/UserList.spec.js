...
import { screen, waitFor, render, router } from 'test/helper'
...
describe('UserList', () => {
  it('renders three users when page loads', async () => {
    render(UserList)
    await waitFor(() => {
  ...
  it('doesn"t display previous page button', async () => {
    render(UserList)
    await screen.findByText('user1')
    expect(
  ...
  describe('when user clicks next', () => {
    ...
    it('displayes previous page button', async () => {
      const { user } = render(UserList)
      await screen.findByText('user1')
      await user.click(screen.queryByRole('button', { name: 'Next' }))
    ...
    describe('when user clicks previous page button', () => {
      it('displays prevous page', async () => {
        const { user } = render(UserList)
        await screen.findByText('user1')
        await user.click(screen.queryByRole('button', { name: 'Next' }))
        await screen.findByText('user4')
    ...
    describe('when user reaches the last page', () => {
      it('doesn"t display next page button', async () => {
        const { user } = render(UserList)
        await screen.findByText('user1')
        await user.click(screen.queryByRole('button', { name: 'Next' }))
        await screen.findByText('user4')
    ...
    describe('when api progress in progress', () => {
